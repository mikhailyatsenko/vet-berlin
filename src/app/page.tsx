      ...
      <main className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
              ...
              <div>
                ...
                <div className="flex gap-2">
                  <select name="neighborhood" defaultValue="" className="w-full px-3 py-2 border border-gray-300 rounded-md focus:ring-2 focus:ring-blue-500 focus:border-transparent">
                    <option value="">All Neighborhoods</option>
                    {neighborhoodsList.map((n) => (<option key={n} value={n}>{n}</option>))}
                  </select>
                  <button type="submit" className="px-4 py-2 border rounded-md">Go</button>
                </div>
              </div>
        ...
        {items.length > 0 ? (
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
            {items.map((v: Veterinarian) => (<VeterinarianCard key={v.googleMapsId} veterinarian={v} />))}
          </div>
        ) : (
      ...
      <footer className="bg-white border-t mt-12">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
          <div className="text-center text-gray-500">
            ...
            <p className="mt-2 text-sm">Data provided by Google Maps • Updated: {new Date().toLocaleDateString('en-US')}</p>
          </div>
        </div>
      </footer>
    </div>
